...
void main() {
    ...
    vec3 highlightColour = vec3(255.0)/255.0;

    vec4 texture = texture2D(wordTexture, vec2((vUv.x - 0.5) * textureRatio/ 1000.0 + 0.5, vUv.y));
    float depth = gl_FragCoord.z / gl_FragCoord.w;
    float fogFactor = smoothstep( fogNear, fogFar, depth);

    gl_FragColor.rgb = mix( highlightColour, vec3(0.0, 0.0, 0.0), mix(0.0, fogFactor, fogIntensity) );
    // gl_FragColor.rgb = colormap(highlightIntensity  ).rgb;

    gl_FragColor.a = texture.a * highlightIntensity;
}
